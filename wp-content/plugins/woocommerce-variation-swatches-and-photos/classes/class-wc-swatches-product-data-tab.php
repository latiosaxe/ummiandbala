...
class WC_Swatches_Product_Data_Tab extends WC_EX_Product_Data_Tab_Swatches {
	...
	public function __construct() {
		parent::__construct( array('swatches', 'show_if_variable'), 'swatches', 'Colores', plugin_dir_url( (dirname( __FILE__ ) ) ) . 'assets/product-data-icon.png' );
	}
		...
		<div class="fields_header">
			<table class="wcsap widefat">
				...
				<th class="attribute_swatch_label">
					<?php _e( 'Nombre de aributo', 'wc_swatches_and_photos' ); ?>
				</th>
				<th class="attribute_swatch_type">
					<?php _e( 'Tipo de Atributo', 'wc_swatches_and_photos' ); ?>
				</th>
				</thead>
			</table>
		</div>
				...
				foreach ( $attribute_names as $name ) :
					...
					if ( $current_options ) {
						...
						if ( $current_type != 'default' ) {
							$current_type_description = ($current_type == 'term_options' ? __( 'Ummi & Bala(Colores solidos)', 'wc_swatches_and_photos' ) : __( 'Thumbnail de imagenes', 'wc_swatches_and_photos' ));
						}
					}
								...
								<table class="wcsap_input widefat wcsap_field_form_table">
									<tbody>
										<tr class="attribute_swatch_type">
											<td class="label">
												<label for="_swatch_type_options_<?php echo $key_attr; ?>_type">Tipo</label>
											</td>
											<td>
												<select class="_swatch_type_options_type" id="_swatch_type_options_<?php echo $key_attr; ?>_type" name="_swatch_type_options[<?php echo $key; ?>][type]">
													<option <?php selected( $current_type, 'default' ); ?> value="default">Ninguno</option>
													<?php if ( $current_is_taxonomy ) : ?>
														<option <?php selected( $current_type, 'term_options' ); ?> value="term_options"><?php _e( 'Ummi and Bala (Colores Solidos)', 'wc_swatches_and_photos' ); ?></option>
													<?php endif; ?>
													<option <?php selected( $current_type, 'product_custom' ); ?> value="product_custom"><?php _e( 'Imagenes de muestra', 'wc_swatches_and_photos' ); ?></option>
												</select>
											</td>
										</tr>
										...
										<tr class="field_option field_option_product_custom field_option_term_options" style="<?php echo $current_type != 'product_custom' && $current_type != 'term_options' ? 'display:none;' : ''; ?>">
											<td class="label">
												<label for="_swatch_type_options_<?php echo $key_attr; ?>_layout">Diseño</label>
											</td>
											<td>
												<?php $layouts = array('default' => __( 'No mostrar', 'wc_swatches_and_photos' ), 'label_above' => __( 'Mostrar abajo de la descripcion', 'wc_swatches_and_photos' )); ?>
										...
										<tr class="field_option field_option_term_default" style="<?php echo $current_type != 'default' ? 'display:none;' : ''; ?>">
											...
											<td>
												<p>
													<?php _e( 'Si el articulo no cuenta con Calor o tamaño, dejar en el valor Ninguno', 'wc_swatches_and_photos' ); ?>
												</p>
											</td>
										</tr>
...
<!--													--><?php //printf( __( 'The color swatch and image configuration will be used from the %s taxonomy.  Navigate to Products -> Attributes to edit the shared swatches and images for this product attribute.', 'wc_swatches_and_photos' ), $current_label ); ?>
													<?php printf( __( 'Para agregar mas valores a %s. Ingresar en Productos > Atributos', 'wc_swatches_and_photos' ), $current_label ); ?>
												</p>
											</td>
										</tr>
												...
												<div class="product_custom">
													...
													<div class="fields_header">
														<table class="wcsap widefat">
															...
															<th class="attribute_swatch_label">
																<?php _e( 'Atributo', 'wc_swatches_and_photos' ); ?>
															</th>
															<th class="attribute_swatch_type">
																<?php _e( 'Tipo', 'wc_swatches_and_photos' ); ?>
															</th>
															</thead>
														</table>
													</div>
