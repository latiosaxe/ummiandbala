 ...
 * Copyright 2014 75nineteen Media LLC.  (email : scott@75nineteen.com)
 *
...
if ( is_woocommerce_active() ) {
    ...
    class WC_Ship_Multiple {
        ...
        function __construct() {
            ...
            //add_action( 'woocommerce_calculate_totals', array( $this, 'calculate_taxes' ), 11 );
            //add_action( 'woocommerce_review_order_before_cart_contents', array( $this, 'calculate_taxes' ) );
        ...
        function product_options() {
            ...
            <p class="form-field method_field">
                <label for="product_method"><?php _e( 'Envío multiple', 'wc_shipping_multiple_address' ); ?></label>
        ...
        function save_address_book() {
            ...
            if ( count($errors) > 0 ) {
                die(json_encode(array( 'ack' => 'ERR', 'errors' => $errors, 'message' => __( 'Por favor ingresa la dirección de envío completa', 'wc_shipping_multiple_address' ))));
            }
        ...
        function address_book() {
            ...
            if (isset($_GET['addressbook']) && $_GET['addressbook'] == 1) {
                ...
                if (!empty($addresses)):
                    ...
                    echo '<h4>'. __( 'No tienes direcciones de envío guardadas', 'wc_shipping_multiple_address' ) .'</h4>';
                endif;
                ?>
                <script type="text/javascript">
                    jQuery(document).ready(function() {
        ...
        function before_checkout_form() {
            ...
            if ( !$has_item_address && $woocommerce->cart->needs_shipping() )  {
                ...
                echo '<p class="woocommerce-info woocommerce_message" id="wcms_message" '. $css .' data-allowed="'. $data .'">
        ...
        function add_item_meta( $meta, $values ) {
            ...
            if ( $methods !== false && !empty($methods) ) {
                if ( isset($values['package_idx']) && isset($packages[$values['package_idx']]) ) {
                    $meta->add( 'Envío multiple', $methods[$values['package_idx']]['label']);
                }
            }
        ...
        function account_addresses() {
            ...
            echo '<div class="form-row">
                    ...
                    <span class="auxiliar YellowLeft"><input type="submit" name="set_addresses" value="'. __( 'Guardar Dirección', 'wc_shipping_multiple_address' ) .'" class="button alt" /></span>
                </div>';
            echo '</form>';
            ?>
            <script type="text/javascript">
        ...
        function render_shipping_row($packages, $type = 2) {
            ...
            if ( $type == 0 || $type == 1):
                ...
                <tr class="multi_shipping">
                    <td style="vertical-align: top; text-align: left" colspan="<?php if ( version_compare( WOOCOMMERCE_VERSION, '2.0', '<' ) ) echo '2'; else echo '1'; ?>">
                        <?php _e( 'Costo de envíos', 'wc_shipping_multiple_address' ); ?>
                    </td>
                ...
                if (! $has_address ) {
                    ...
                    <td>
                        <div class="ship_address no_shipping_address">
                            ...
                            echo '<p style="text-align: center"><a href="'. get_permalink($page_id) .'" class="button modify-address-button">'. __( 'Assign Shippping Address', 'wc_shipping_multiple_address' ) .'</a></p>';
                            //}
                            ?>
                        </div>
                    </td>
                ...
                ?>
                    <td>
                        <div class="ship_address">
                            ...
                            $sess_cart_addresses = wcms_session_get( 'cart_item_addresses' );
                            if ( $sess_cart_addresses && !empty($sess_cart_addresses) ) {
                                echo '<p><a href="'. get_permalink($page_id) .'" class="modify-address-button">'. __( 'Modificar', 'wc_shipping_multiple_address' ) .'</a></p>';
                            }
                            ?>
                        </div>
                    </td>
            ...
            <?php
                ...
                <tr class="multi_shipping">
                    <td style="vertical-align: top;" colspan="<?php if ( version_compare( WOOCOMMERCE_VERSION, '2.0', '<' ) ) echo '2'; else echo '1'; ?>">
                        <?php _e( 'Envío multiple', 'wc_shipping_multiple_address' ); ?>
        ...
        function shipping_packages($packages) {
            ...
            $this->generate_address_session( $packages );
...
//                return;
            ...
            if (isset($_POST['action']) && $_POST['action'] == 'woocommerce_update_shipping_method') return $cart;
        ...
        public function calculate_taxes( $cart = null, $packages = null ) {
            ...
            $old_total                  = max( 0, apply_filters( 'woocommerce_calculated_total', round( $cart->cart_contents_total + $cart->tax_total + $cart->shipping_tax_total + $cart->shipping_total - $cart->discount_total + $cart->fee_total, $cart->dp ), $cart ) );
            ...
            $woocommerce->customer->set_shipping_location(
                $default_shipping_location[0],
                $default_shipping_location[1],
                $default_shipping_location[2]
            );
