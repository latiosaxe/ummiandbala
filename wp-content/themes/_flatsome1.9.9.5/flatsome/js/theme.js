...
jQuery(document).ready(function(){
    ...
    if(jQuery('.multi_shipping').length<=0){
        jQuery(".shop_table tfoot").addClass("empty");
    }
    if(jQuery('option[value="free_shipping"]').length>0){
        jQuery("option[value='free_shipping']").attr("selected","selected");
    }
});
...
jQuery(document).ready(function(){
    ...
    jQuery(document).ready(function () {
        function init() {
            var input_name = "";
            jQuery.each( jQuery("form input, form textarea"), function() {
    ...
    jQuery('#formCheckOut').submit(function() {
        localStorage.clear();
    });
    ...
    jQuery(document).ready(function () {
        if(jQuery('.multi_shipping').length<=0){
            jQuery(".auxMultiple").addClass('hidden');
        }
    });
}(jQuery));

function triggerMiltuple(este){
    var este = este;
    if(jQuery(".multi_shipping").length>0){
        jQuery(".auxMultiple").hide();
    }else{
        if(este.id == "multipleYes"){
            jQuery("#payment").hide().removeClass('hidden').slideDown(500);
        }else if(este.id == "multipleNo"){
            document.location.href = '/terminar-compra/direcciones-de-envio/';
        }
    }
}

function after_update_checkout(){
    var este = jQuery('input[name=multipleRadio]:checked', '#formCheckOut').val();
    if(jQuery(".multi_shipping").length>0){
        jQuery(".auxMultiple").hide();
